...
export default function NewProductPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () => {
      ...
      // basePrice = lowest variant price so the storefront listing price is always accurate
  ...
  const handleNameChange = (value: string) => {
    setField('name', value);
    if (!form.slug || form.slug === autoSlug(form.name)) setField('slug', autoSlug(value));
  };
  ...
  const addVariant = () => setVariants((c) => [...c, emptyVariant()]);
  const removeVariant = (index: number) => setVariants((c) => c.filter((_, i) => i !== index));

  const canSubmit =
    form.name &&
    form.slug &&
    variants.some((v) => v.name && v.value && v.price);
    ...
    <div className="admin-page">
      <section className="admin-panel p-6 sm:p-7">
        <div className="flex flex-wrap items-start gap-4">
          ...
          <div>
            <p className="admin-eyebrow">Catalog creation</p>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-[#5E5C54]">
              Build a product with cleaner editorial structure, variant control, and content
              fields that match the storefront&s luxury tone.
            </p>
          </div>
        </div>
      </section>
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1">
                {fieldErrors.map((msg, i) => (
                  <li key={i} className="text-sm text-[#7A3A1A]">
                    · {msg}
                  </li>
                ))}
              </ul>
            ...
            <div className="mt-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Medjool Gold Dates"
              ...
              <div>
                ...
                />
                <p className="mt-1.5 text-xs text-[#7A776F]">Auto-generated from name. Used in the product URL.</p>
              </div>
              <div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-xs text-[#7A776F]">
                  Each variant needs a <strong>Name</strong> (label shown to customer), a <strong>Value</strong> (e.g. 500g, Red, Large), a <strong>Price</strong>, and a <strong>Stock</strong> count.
                </p>
              </div>
              ...
              {variants.map((variant, index) => (
                <div key={index} className="rounded-[1.4rem] border border-[#EDE5DA] bg-white/60 p-4">
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                    {([
                      { key: 'name' as keyof Variant, label: 'Name *', placeholder: '500g Box', hint: 'Label shown in the product page' },
                      { key: 'value' as keyof Variant, label: 'Value *', placeholder: '500g', hint: 'Machine-readable value (e.g. 500g, red)' },
                    ...
                    ] as { key: keyof Variant; label: string; placeholder: string; hint: string }[]).map(({ key, label, placeholder, hint }) => (
                      <div key={key}>
          ...
          <div className="admin-panel p-6">
            ...
            <div className="mt-6">
              ...
              <textarea
                ...
                rows={5}
                placeholder={`https://res.cloudinary.com/vibio/image/upload/v1/product-1.jpg\nhttps://res.cloudinary.com/vibio/image/upload/v1/product-1-2.jpg`}
                className="admin-textarea resize-none font-mono text-xs"
              />
              <p className="mt-1.5 text-xs text-[#7A776F]">Use Cloudinary or any public image URL. First image is the product thumbnail.</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
            ...
            <div className="mt-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="admin-input"
                />
                <p className="mt-1.5 text-xs text-[#7A776F]">Comma-separated. Used for filtering and search.</p>
              </div>
          ...
          <div className="admin-panel p-4">
            <p className="text-xs text-[#7A776F]">
              <strong className="text-[#3E433E]">Base price</strong> is set automatically from the lowest variant price and shown as the listing price in the storefront.
            </p>
          </div>
          ...
          {fieldErrors.length > 0 && (
            <p className="text-center text-xs text-[#C9571A]">
              Scroll up to see the errors
            </p>
          )}
